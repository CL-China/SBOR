function [w,used,by,logMarginalLog,sigma,e] = ISBOR(train_x,train_y,OPTIONS)
...
%     if i== OPTIONS.iterations
    ...
    if i==OPTIONS.iterations || i>10 &&...
            abs(logMarginalLog(count)-logMarginalLog(count-1)) < ...
            1e-2/M_full*abs(logMarginalLog(count-1))
...
for i = 1:T 
    ...
    idx = [idx;tmp_idx(bar(1:ceil(y_n(i)/N*T*5)))];
end
used = idx;
PHI = K(:,used);
phi = PHI(used,:);
